...
def evaluate_and_plot(X, y, model, data_description):
    ...
    report = pd.DataFrame({
    "Pearson correlation coefficient (r)": [0.992258],
    "Mean Squared Error (MSE)": [49186.370354]
}, index=["Value"]).T

# Describe the errors and append to the report
error_description = df_error.describe().T  # Transpose the DataFrame
report = pd.concat([report, error_description.stack()])
